...
class SecurityLevelInterceptor(
    ...
    companion object {
        ...
        @Keep
        val keysByNspace = ConcurrentHashMap<Long, Key>()

        @Keep
        val patchedResponses = ConcurrentHashMap<Key, KeyEntryResponse>()
        ...
        fun findGeneratedKey(uid: Int, descriptor: KeyDescriptor): Info? {
            if (descriptor.nspace != 0L) {
                findGeneratedKeyByKeyId(uid, descriptor.nspace)?.let { return it }
            }
        ...
        fun resolvePatchedResponse(uid: Int, descriptor: KeyDescriptor): KeyEntryResponse? {
            if (descriptor.nspace != 0L) {
                keysByNspace[descriptor.nspace]?.takeIf { it.uid == uid }?.let { return patchedResponses[it] }
            }
        ...
        fun shouldSkipLeafHackFor(uid: Int, descriptor: KeyDescriptor): Boolean {
            if (descriptor.nspace != 0L) {
                keysByNspace[descriptor.nspace]?.takeIf { it.uid == uid }?.let { return skipLeafHacks[it] ?: false }
            }
            return descriptor.alias?.let { skipLeafHacks[Key(uid, it)] } ?: false
        }

        @Keep
        val usageRemaining = ConcurrentHashMap<Key, Int>()
    ...
    data class Info(val keyPair: KeyPair?, val secretKey: javax.crypto.SecretKey?, val response: KeyEntryResponse, val params: CertificateGen.KeyGenParameters)
        ...
        if (code == generateKeyTransaction) {
            ...
            kotlin.runCatching {
                ...
                if (challenge != null && challenge.size > MAX_ATTESTATION_CHALLENGE_BYTES) {
                    Logger.i("Rejecting oversized attestation challenge (${challenge.size}B > $MAX_ATTESTATION_CHALLENGE_BYTES) uid=$callingUid alias=${keyDescriptor.alias}")
    ...
    private fun storeGeneratedKey(
        callingUid: Int, keyDescriptor: KeyDescriptor, kgp: CertificateGen.KeyGenParameters,
        keyPair: KeyPair?, secretKey: javax.crypto.SecretKey?, chain: List<Certificate>?,
    ...
    private fun buildResponse(
        ...
        val d = KeyDescriptor()
        d.domain = 4 // Domain.KEY_ID — the real keystore normalizes APP to KEY_ID after generation
        d.nspace = descriptor.nspace
        d.alias = null  // alias is cleared when domain is KEY_ID
        metadata.key = d
